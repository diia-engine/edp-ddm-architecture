...
Для вдалого встановлення кластера та платформи, потрібно підняти наступні ресурси в AWS. На малюнку нижче зображена схема інфраструктури із ними. Це зроблено для спрощення інсталяції платформи та уникнення небажаних помилок, які можуть бути пов’язані з встановленням з локального комп’ютеру.
...
* S3 кошик – використовується для зберігання стану терраформу.
* DynamoDB - таблиця в DynamoDB, яка буде використовуватися для збереження інформації про блокування стану терраформу.
* NAT Gateway - використовується для забезпечення приватного серверу доступом до Інтернету.
* Bastion - використовується як проміжний сервер для забезпечення безпечного та обмеженого доступу до серверу у приватній мережі. Надалі, через цей бастіон буде прокинуто SSH-тунель до initial-node.
* Initial-node -  сервер у приватній мережі, через який буде відбуватися інсталяція кластеру та платформи.

Розгорнути ці ресурси можна за допомогою підготовленного Terraform-коду в наступних кроках.
...
. Після цього необхідно звернутися до команди підтримки, які повинні додати ID облікового запису AWS в Trust relationships для ролі `CustomCrossAccountRole` в їх обліковому записі AWS.
...
** bucket - ця змінна містить ім’я S3 бакету. Змініть <ACCOUNT_ID> на ID від облікового запису AWS.
* `iam-node-role.tf` - використовується для створення спеціальної IAM-ролі із необхідними дозволами. Це дасть змогу налаштувати AWS cross account resource access та завантажити Docker-образ для контейнеру та Інсталеру.
* `elastic-ip.tf` – використовується для створення ресурсу AWS Elastic IP (EIP) за допомогою Terraform.
* `security-groups.tf` - створюються Security Groups, які дозволя.nm SSH-з'єднання (TCP порт 22) для бастіону та initial-node.
* `ssh-key.tf` - містить код для створення SSH приватного ключа та збереження ключа у файл та налаштування його прав доступу.
* `files/user_data.sh.tpl` - шаблон скрипта, який буде виконуватися при створенні або оновленні EC2 інстансу в середовищі AWS. Цей скрипт зробить наступне для initial-node:
** встановить Docker;
** встановить Unzip;
...
* `terraform.tfvars` - містить значення для конкретних змінних, які визначені у конфігураційних файлах Terraform. За потреби змініть значення для наступних параметрів на необхідні:
** region – ця змінна використовується для визначення регіону AWS, в якому будуть створюватися ресурси.
** platform_name - ця змінна використовується для додавання назви для кластеру та ресурсів AWS.
** ingress_cidr_blocks - для підключення через SSH до додаткової віртуальної машини потрібно додати сюди необхідну IP адресу.
...
[source,bash]
----
<<<<<<< HEAD   (e9e32c [MDTUDDM-00000] added fixes to special-steps 1.9.6)
...
>>>>>>> CHANGE (b7665d [MDTUDDM-28890] Platform deployment instructions have been m)
----
...
Після виконання усіх дій, бастіон та додаткову віртуальну машину можна вимкнути. Вони не будуть потрібні до наступного оновлення платформи.
====
