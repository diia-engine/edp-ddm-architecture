...
. Obtain consistent data in registry bucket backups during migration.
...
.. Open each *CronJob* and change its start time to the nearest possible time. For example, set it to run in the next 10-15 minutes. To do this, go to the settings of each CronJob, open its *YAML* configuration, and change the `spec.schedule` parameter. For example, to run a CronJob daily at 10:50 UTC, use the following configuration:
+
...
.. By following these steps, you will be able to obtain consistent data from the backups of registry buckets in the migration process.
